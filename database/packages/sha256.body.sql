...
*/
    ...
    FUNCTION hashpw(password IN VARCHAR2, salt IN VARCHAR2) RETURN VARCHAR2 AS
    BEGIN
        RETURN '$5$'||salt||ENCRYPT(password);
    END;

    FUNCTION gensalt RETURN VARCHAR2 AS
    BEGIN
        RETURN dbms_random.string('A',72);
    END;

    FUNCTION checkpw(plaintext IN VARCHAR2, hashed IN VARCHAR2) RETURN BOOLEAN AS
        v_salt VARCHAR2(72);
    BEGIN
        v_salt := substr(substr(hashed,4),1,72); -- remove prefix $5$ and get salt
        RETURN (hashpw(plaintext,v_salt) = hashed);
    END;
