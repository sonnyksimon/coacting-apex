...
function p149_RadioTog(){
 var curVal = html_RadioValue('P149_CONST_TYPE'),
     suffix = curVal === 'C'
              ? '_ck'+g_conCount
              : '_uk'+g_ukCount,
     tableName = html_GetElement('P149_TABLE_NAME').value,
     conName;
 //
 // Compute conName by appending suffix to tableName, but chop off chars from
 // tableName to make the result 30 or 128 chars long. Since we do not know on
 // the client whether long identifiers are supported, we have to guess whether
 // chars have to be chopped off.
 //
 if (tableName.length+suffix.length <= 30) {
  conName = tableName+suffix;
 } else if (tableName.length <= 30) {
  conName = tableName.substr(0, 30-suffix.length) + suffix;
 } else if (tableName.length+suffix.length <= 128) {
  conName = tableName+suffix;
 } else {
  conName = tableName.substr(0, 128-suffix.length) + suffix;
 }

 html_ShowItemRow('P149_CHECK');
 html_HideItemRow('P149_AVAIL_COLS');
 html_HideItemRow('P149_SEL_COLS');
 html_GetElement('P149_NAME').value = conName;
}
