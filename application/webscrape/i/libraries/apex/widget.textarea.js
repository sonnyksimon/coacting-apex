...
widget.textarea = function( pId, pOptions ) {
    ...
    function resizable() {
        ...
        function startResize( pEvent ) {
            ...
            $( document )
                .on( "mousemove.apex_startResize", function( pE ){ return performResize(pE, ( $(pEvent.currentTarget).css( "cursor" ) === "se-resize" ));})
    ...
    apex.item.create( pId, {
        show: function() {
            gTextarea.closest( ".apex-item-group--textarea" ).show();
        },
        hide: function() {
            gTextarea.closest( ".apex-item-group--textarea" ).hide();
        }
    });
