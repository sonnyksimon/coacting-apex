...
tree.Comment.prototype = {
    type: "Comment",
    genCSS: function (env, output) {
        if (!less.omitComments && this.debugInfo) {
            output.add(tree.debugInfo(env, this), this.currentFileInfo, this.index);
        }
        !less.omitComments && output.add(this.value.trim()); //TODO shouldn't need to trim, we shouldn't grab the \n
    },
    toCSS: tree.toCSS,
    ...
    eval: function () { 
        ...
        // Added by miguel
        if (this.value.indexOf("\"var\"") > -1) {
            try {
                var variable = null;
                var properties = JSON.parse(this.value.replace(new RegExp("^\\s*/\\*\\s*"), "").replace(new RegExp("\\s*\\*/\\s*$"), ""));
                
                variable = properties.var;
                delete(properties.var);
                
                less.vars = less.vars || {};
                less.vars[variable] = less.vars[variable] || {};
                
                for (property in properties) {
                    if(properties.hasOwnProperty(property)){
                        if(property === "paletteBind"){
                            less.paletteVars = less.paletteVars || [];
                            less.paletteVars[properties[property]] = less.paletteVars[properties[property]] || [];
                            if(less.paletteVars[properties[property]].indexOf(variable) === -1){
                                less.paletteVars[properties[property]].push(variable);
                            }
                        } else {
                            less.vars[variable][property] = properties[property];
                        }
                    }
                }
            } catch (e) {
        ...
        if (this.value.indexOf("\"groups\"") > -1) {
            try {
                var object = JSON.parse(this.value.replace(new RegExp("^\\s*/\\*\\s*"), "").replace(new RegExp("\\s*\\*/\\s*$"), ""));
                less.groups = object.groups;

            } catch (e) {
                less.groups = less.groups || [];
            }
        }
        if (this.value.indexOf("\"palette\"") > -1) {
            try {
                var object = JSON.parse(this.value.replace(new RegExp("^\\s*/\\*\\s*"), "").replace(new RegExp("\\s*\\*/\\s*$"), ""));
                less.palette = object.palette;

            } catch (e) {
                less.palette = less.palette || {};
            }
        }
        if (this.value.indexOf("\"translate\"") > -1) {
            try {
                var object = JSON.parse(this.value.replace(new RegExp("^\\s*/\\*\\s*"), "").replace(new RegExp("\\s*\\*/\\s*$"), ""));
                less.translate = !!object.translate;

            } catch (e) {
                less.translate = !!less.translate;
            }
        }
...
tree.Rule.prototype = {
    ...
    eval: function (env) {
        ...
        try {
            ...
            if(this.variable){
                less.vars = less.vars || {};
                less.vars[this.name] = less.vars[this.name] || {};
                less.vars[this.name].value = evaldValue.toCSS(env);
            }
...
tree.Variable.prototype = {
    ...
    eval: function (env) {
        ...
        if (variable) { 
            ...
            // Added by andres
            less.vars = less.vars || {};
            less.vars[name] = less.vars[name] || {};
            less.vars[name].last = variable.toCSS(env);
...
// Added by andlozan
// This flag will prevent comment nodes from showing on the output
less.omitComments = less.omitComments || false;
...
function compile(lessCode, modifyVars, callback, callback2) {
    delete less.groups;
    delete less.palette;
    delete less.translate;
    var env = new less.tree.parseEnv(less);
    env.filename = document.location.href.replace(/#.*$/, '');
    ...
    var cb = function (e, cssAST) {
        ...
        // Normalize less.vars, less.groups and less.palette
        less.palette = less.palette || {};
        less.translate = !!less.translate;
        if (typeof less.palette.enabled === 'undefined')
            // If false, the palette generator will not be shown for this style
            less.palette.enabled = false;
        if (typeof less.palette.availableModes === 'undefined')
            less.palette.availableModes = [
                "monochrome",										// 1 output 
                "monochrome+c", "monochrome+l", "monochrome+r",		// 2 outputs
                "triad",											// 3 outputs
                "triad+c", "tetrad"									// 4 outputs
            ];
        if (typeof less.palette.autoFillOutputs === 'undefined')
            // If true, the palette will ensure a full 4-color output even if the user picks an option with less outputs.
            // Otherwise, unused variables will be set to their default LESS values.
            less.palette.autoFillOutputs = true;
        if (typeof less.palette.userDefinedShades === 'undefined')
            // This will make the controls for shades show up in the UI
            less.palette.userDefinedShades = 5;
        if (typeof less.palette.defaults === 'undefined')
            // This will make the controls for shades show up in the UI
            less.palette.defaults = {
                hue: 250,
                separation: 20,
                mode: "triad"
            };
        if (typeof less.palette.outputMapping === 'undefined')
            // Palette outputs will be placed into these LESS variables automatically. The outputs will be treated
            // as normal user changes, and thus can override user-set values and be overriden by user actions.
            // Every 5 outputs (0-4, 5-9, etc) are different shades for the same hue.
            less.palette.outputMapping = {
        ...
        for (var i = less.groups.length - 1; i >= 0; i--) {
            if ( less.groups[i].common === undefined ) {
                less.groups[i].common = true;
            }
        };

        var all_groups = less.groups.map(function(d){ return d.name; });
        var v;
        for (var k in less.vars) {
            v = less.vars[k];
            if ( v.common === undefined ) {
                v.common = true;
            }
            if ( v.group && ( all_groups.indexOf(v.group) < 0 ) ) {
                // If v.group doesn't have a formal definition, create one with default values
                less.groups.push({
                    name: v.group,
                    sequence: 9999,
                    common: true
                });
                all_groups.push( v.group );
            }
        }
